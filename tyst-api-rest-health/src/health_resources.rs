...
/** Health check status definitions according to Eclipse MicroProfile Health 3.1.
...
/** HTTP response body object for health requests. Only basic status is
supported.
 */
...
pub async fn health(app_health: Data<Arc<dyn AppHealth>>) -> impl Responder {
    ...
    if app_health.is_health_started() && app_health.is_health_ready() && app_health.is_health_live()
    {
...
/** This endpoint returns the liveness of a microservice, or whether it
...
pub async fn health_started(app_health: Data<Arc<dyn AppHealth>>) -> impl Responder {
    if app_health.is_health_started() {
        HealthStatus::Up.as_response()
    } else {
        HealthStatus::Down.as_response()
    }
